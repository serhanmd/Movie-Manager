...
class ListAdapter(private val context: Context, var movies: ArrayList<Movie>):BaseAdapter() {
    ...
    fun checkIfFavorite(viewHolder: ViewHolder, position: Int) = when (movies[position].isFavorite)  {
        movies[position].isFavorite -> viewHolder.favoriteImage?.setImageResource(R.drawable.favorite)
        else -> viewHolder.favoriteImage?.setImageResource(R.drawable.notfavorite)
    }
